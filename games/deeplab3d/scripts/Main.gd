extends Node3D
## MechLab3D shell: a museum of deep / systemic 3D mechanics. Registry-driven;
...
const DEMOS := {
	"actionrpg": {"title": "ACTION RPG", "era": "3rd-person combat + levels", "script": preload("res://scripts/demos/ActionRPG.gd")},
	"fpsarena": {"title": "FPS ARENA", "era": "move+look+shoot waves", "script": preload("res://scripts/demos/FPSArena.gd")},
	"td3d": {"title": "TOWER DEFENSE 3D", "era": "build + creeps path", "script": preload("res://scripts/demos/TD3D.gd")},
	"racing": {"title": "CIRCUIT RACING", "era": "laps + AI + drift boost", "script": preload("res://scripts/demos/Racing.gd")},
	"platform3d": {"title": "PLATFORMER 3D", "era": "collectathon + double jump", "script": preload("res://scripts/demos/Platform3D.gd")},
	"dogfight": {"title": "DOGFIGHT", "era": "3D plane air combat", "script": preload("res://scripts/demos/Dogfight.gd")},
	"citybuild3d": {"title": "CITY BUILDER 3D", "era": "zones + economy (orbit cam)", "script": preload("res://scripts/demos/CityBuild3D.gd")},
	"bossraid": {"title": "BOSS RAID", "era": "patterns + phases + dodge", "script": preload("res://scripts/demos/BossRaid.gd")},
	"survival3d": {"title": "SURVIVAL 3D", "era": "gather + craft + night", "script": preload("res://scripts/demos/Survival3D.gd")},
	"mech": {"title": "MECH COMBAT", "era": "drive + turret + waves", "script": preload("res://scripts/demos/Mech.gd")},
}
const ORDER := ["actionrpg", "fpsarena", "td3d", "racing", "platform3d",
...
func _build_ui() -> void:
	...
	_lbl(v, "DEEPLAB 3D", 68, Color.WHITE)
	_lbl(v, "deep / systemic 3D mechanics · pick an exhibit", 24, Color(1, 1, 1, 0.55))
	var sp := Control.new()
