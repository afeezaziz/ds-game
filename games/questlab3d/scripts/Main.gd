extends Node3D
## questlab3d shell: a museum of FLAGSHIP 3D genres — the shapes behind the
...
const DEMOS := {
	"jrpg": {"title": "TURN-BASED JRPG", "era": "party · ATB · skills", "script": preload("res://scripts/demos/Jrpg.gd")},
	"squadbreach": {"title": "SQUAD BREACH", "era": "command · cover · clear", "script": preload("res://scripts/demos/SquadBreach.gd")},
	"zeldadungeon": {"title": "DUNGEON QUEST", "era": "items unlock new paths", "script": preload("res://scripts/demos/ZeldaDungeon.gd")},
	"metroidvania": {"title": "METROIDVANIA 3D", "era": "ability-gated map", "script": preload("res://scripts/demos/Metroidvania.gd")},
	"survivalhorror": {"title": "SURVIVAL HORROR", "era": "scarcity · scares", "script": preload("res://scripts/demos/SurvivalHorror.gd")},
	"immersivesim": {"title": "IMMERSIVE SIM", "era": "many paths · powers", "script": preload("res://scripts/demos/ImmersiveSim.gd")},
	"dungeoncrawl": {"title": "DUNGEON CRAWLER", "era": "first-person grid", "script": preload("res://scripts/demos/DungeonCrawl.gd")},
}
const ORDER := ["jrpg", "squadbreach", "zeldadungeon", "metroidvania", "survivalhorror", "immersivesim", "dungeoncrawl"]
...
func _build_ui() -> void:
	...
	_lbl(v, "QUESTLAB 3D", 68, Color.WHITE)
	_lbl(v, "3D RPG & adventure · pick an exhibit", 24, Color(1, 1, 1, 0.55))
	var sp := Control.new()
