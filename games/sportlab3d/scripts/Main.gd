extends Node3D
## sportlab3d shell: a museum of FLAGSHIP 3D genres — the shapes behind the
...
const DEMOS := {
	"golf": {"title": "GOLF", "era": "aim · power · terrain · putt", "script": preload("res://scripts/demos/Golf.gd")},
	"pool": {"title": "POOL", "era": "angle · spin · sink the rack", "script": preload("res://scripts/demos/Pool.gd")},
	"bowling": {"title": "BOWLING", "era": "line · hook · pin physics", "script": preload("res://scripts/demos/Bowling.gd")},
	"skate": {"title": "SKATEBOARDING", "era": "trick combos · balance · grind", "script": preload("res://scripts/demos/Skate.gd")},
	"snowsurf": {"title": "SNOWBOARD", "era": "carve · air · flow downhill", "script": preload("res://scripts/demos/SnowSurf.gd")},
	"fishing": {"title": "FISHING", "era": "cast · hook · reel tension", "script": preload("res://scripts/demos/Fishing.gd")},
	"boxing": {"title": "BOXING", "era": "slip · block · combo timing", "script": preload("res://scripts/demos/Boxing.gd")},
}
const ORDER := ["golf", "pool", "bowling", "skate", "snowsurf", "fishing", "boxing"]
...
func _build_ui() -> void:
	...
	_lbl(v, "SPORTLAB 3D", 68, Color.WHITE)
	_lbl(v, "3D sports & physics toys · pick an exhibit", 24, Color(1, 1, 1, 0.55))
	var sp := Control.new()
