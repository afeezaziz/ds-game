extends Node3D
## shootlab3d shell: a museum of FLAGSHIP 3D genres — the shapes behind the
...
const DEMOS := {
	"extraction": {"title": "EXTRACTION SHOOTER", "era": "raid · loot · extract or lose it", "script": preload("res://scripts/demos/Extraction.gd")},
	"hero": {"title": "HERO SHOOTER", "era": "asymmetric kits · objective", "script": preload("res://scripts/demos/Hero.gd")},
	"onrails": {"title": "RAIL SHOOTER", "era": "on-rails · aim · light-gun", "script": preload("res://scripts/demos/RailShooter.gd")},
	"horde": {"title": "HORDE CO-OP", "era": "waves · specials · hold out", "script": preload("res://scripts/demos/Horde.gd")},
	"fpstd": {"title": "FPS TOWER DEFENSE", "era": "traps + gun · defend the lane", "script": preload("res://scripts/demos/FpsTd.gd")},
}
const ORDER := ["extraction", "hero", "onrails", "horde", "fpstd"]
...
func _build_ui() -> void:
	...
	_lbl(v, "SHOOTLAB 3D", 68, Color.WHITE)
	_lbl(v, "3D shooter genres · pick an exhibit", 24, Color(1, 1, 1, 0.55))
	var sp := Control.new()
