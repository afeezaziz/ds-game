extends Node3D
## tablelab3d shell: a museum of FLAGSHIP 3D genres — the shapes behind the
...
const DEMOS := {
	"cardbattler": {"title": "CARD BATTLER", "era": "mana · minions · lethal", "script": preload("res://scripts/demos/CardBattler.gd")},
	"autobattler": {"title": "AUTO BATTLER", "era": "draft · position · combine", "script": preload("res://scripts/demos/AutoBattler.gd")},
	"partygames": {"title": "PARTY GAMES", "era": "minigame gauntlet", "script": preload("res://scripts/demos/PartyGames.gd")},
	"physicspuzzle": {"title": "PHYSICS PUZZLE", "era": "build · solve · escape", "script": preload("res://scripts/demos/PhysicsPuzzle.gd")},
}
const ORDER := ["cardbattler", "autobattler", "partygames", "physicspuzzle"]
...
func _build_ui() -> void:
	...
	_lbl(v, "TABLELAB 3D", 68, Color.WHITE)
	_lbl(v, "3D board / card / party · pick an exhibit", 24, Color(1, 1, 1, 0.55))
	var sp := Control.new()
