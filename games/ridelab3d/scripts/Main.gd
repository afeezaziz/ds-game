extends Node3D
## ridelab3d shell: a museum of FLAGSHIP 3D genres — the shapes behind the
...
const DEMOS := {
	"trucksim": {"title": "DELIVERY DRIVER", "era": "cargo runs · traffic · time", "script": preload("res://scripts/demos/Delivery.gd")},
	"tankbattle": {"title": "TANK BATTLE", "era": "aim · armor angles · shells", "script": preload("res://scripts/demos/TankBattle.gd")},
	"spaceflight": {"title": "SPACE FLIGHT", "era": "6DOF Newtonian dogfight", "script": preload("res://scripts/demos/SpaceFlight.gd")},
	"derby": {"title": "DEMOLITION DERBY", "era": "ram · deform · last running", "script": preload("res://scripts/demos/Derby.gd")},
}
const ORDER := ["trucksim", "tankbattle", "spaceflight", "derby"]
...
func _build_ui() -> void:
	...
	_lbl(v, "RIDELAB 3D", 68, Color.WHITE)
	_lbl(v, "3D vehicle genres · pick an exhibit", 24, Color(1, 1, 1, 0.55))
	var sp := Control.new()
