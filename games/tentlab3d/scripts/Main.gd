extends Node3D
## tentlab3d shell: a museum of FLAGSHIP 3D genres — the shapes behind the
## top-grossing charts (MOBA, battle royale, open-world, voxel, cover shooter,
## stealth, musou, grapple, rocket-car, RTS). Registry-driven; each demo is a
## self-contained Node3D (own camera + world). Per-demo leaderboard (board ==
## key), local best, analytics tag. UI is a CanvasLayer overlay over the 3D scene.

enum State { MENU, PLAYING, OVER }

const DEMOS := {
	"rhythm": {"title": "RHYTHM", "era": "lanes · beat · combo", "script": preload("res://scripts/demos/Rhythm.gd")},
	"partyroyale": {"title": "PARTY ROYALE", "era": "obstacle gauntlet · qualify", "script": preload("res://scripts/demos/PartyRoyale.gd")},
	"factory": {"title": "FACTORY", "era": "belts · machines · throughput", "script": preload("res://scripts/demos/Factory.gd")},
	"soccer": {"title": "SOCCER", "era": "dribble · pass · shoot", "script": preload("res://scripts/demos/Soccer.gd")},
	"basketball": {"title": "BASKETBALL", "era": "drive · arc shot · defend", "script": preload("res://scripts/demos/Basketball.gd")},
	"pinball": {"title": "PINBALL", "era": "flippers · bumpers · 3 balls", "script": preload("res://scripts/demos/Pinball.gd")},
	"portal": {"title": "PORTAL", "era": "place linked portals · cross", "script": preload("res://scripts/demos/Portal.gd")},
	"perspective": {"title": "PERSPECTIVE", "era": "rotate to align the path", "script": preload("res://scripts/demos/Perspective.gd")},
	"lootarpg": {"title": "LOOT ARPG", "era": "kill · loot · upgrade", "script": preload("res://scripts/demos/LootARPG.gd")},
	"souls": {"title": "SOULS-LIKE", "era": "stamina · roll · estus · boss", "script": preload("res://scripts/demos/Souls.gd")},
	"tactics": {"title": "TACTICS", "era": "grid · cover · flank", "script": preload("res://scripts/demos/Tactics.gd")},
	"photomode": {"title": "PHOTO SAFARI", "era": "frame · time · snap", "script": preload("res://scripts/demos/PhotoMode.gd")},
}
const ORDER := ["rhythm", "partyroyale", "factory", "soccer", "basketball", "pinball", "portal", "perspective", "lootarpg", "souls", "tactics", "photomode"]
...
func _build_ui() -> void:
	...
	_lbl(v, "TENTLAB 3D", 68, Color.WHITE)
	_lbl(v, "tentpole flagship gaps · pick an exhibit", 24, Color(1, 1, 1, 0.55))
	var sp := Control.new()
