extends Node3D
## simlab3d shell: a museum of FLAGSHIP 3D genres — the shapes behind the
...
const DEMOS := {
	"farmlife": {"title": "FARM & LIFE SIM", "era": "plots · seasons · townsfolk", "script": preload("res://scripts/demos/FarmLife.gd")},
	"cooking": {"title": "COOKING RUSH", "era": "orders · stations · timers", "script": preload("res://scripts/demos/Cooking.gd")},
	"tycoon": {"title": "PARK TYCOON", "era": "build · guests · economy", "script": preload("res://scripts/demos/Tycoon.gd")},
	"godcolony": {"title": "COLONY SIM", "era": "assign · survive · grow", "script": preload("res://scripts/demos/Colony.gd")},
	"monstertamer": {"title": "MONSTER TAMER", "era": "catch · team · battle", "script": preload("res://scripts/demos/MonsterTamer.gd")},
}
const ORDER := ["farmlife", "cooking", "tycoon", "godcolony", "monstertamer"]
...
func _build_ui() -> void:
	...
	_lbl(v, "SIMLAB 3D", 68, Color.WHITE)
	_lbl(v, "3D sim & management · pick an exhibit", 24, Color(1, 1, 1, 0.55))
	var sp := Control.new()
